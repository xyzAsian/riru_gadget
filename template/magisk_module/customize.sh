...
if [ "$ARCH" = "x86" ] || [ "$ARCH" = "x64" ]; then
  ...
  extract "$ZIPFILE" "system/app/GGManager/lib/arm/libxyzggmanager.so" "$MODPATH"
  ...
  if [ "$IS64BIT" = true ]; then
    ...
    extract "$ZIPFILE" "system/app/GGManager/lib/arm64/libxyzggmanager.so" "$MODPATH"
  fi

  extract "$ZIPFILE" "system/app/GGManager/GGManager.apk" "$MODPATH"
fi
